---
title: Multimodal AI / 多模态 AI
type: topic
area: knowledge
status: active
aliases:
  - Multimodal AI
  - Vision-Language Models
  - 多模态 AI
  - 视觉语言模型
tags:
  - area/knowledge
  - topic/multimodal-ai
  - type/topic
  - wiki/af
---
# Multimodal AI / 多模态 AI

## Definition

Multimodal AI covers models and agents that reason over text, images, video, vision-language grounding, spatial-temporal scenes, and other non-text signals.

In this wiki, the topic is connected to agent systems when perception becomes part of action, tool use, or evidence-grounded reasoning.

## Why it matters

- Multimodal perception is a key path toward agentic vision and real-world interaction.
- It connects evaluation, grounding, data composition, and tool use.
- It creates additional safety and traceability requirements for evidence-based agent behavior.

## Source References

- `baidu-sync:d4be2ee348e4` — CLIP_Wont_Learn_Object_Attribute_Binding_from_Natural_Data_and_Here_is_Why_2507.07985v1.pdf
- `baidu-sync:98ad2439b37b` — Impact_of_Pretraining_Word_Co_occurrence_on_Compositional_Generalization_in_Multimodal_Models_2507.08000v1.pdf
- `baidu-sync:ab88db60c7c3` — PyVision_Agentic_Vision_with_Dynamic_Tooling_2507.07998v1.pdf
- `baidu-sync:ce38b2926bc6` — Traceable_Evidence_Enhanced_Visual_Grounded_Reasoning_Evaluation_and_Methodology_2507.07999v1.pdf
- `baidu-sync:607725b35dc3` — OST_Bench_Evaluating_the_Capabilities_of_MLLMs_in_Online_Spatio_temporal_Scene_Understanding_2507.07984v1.pdf

## Related topics

- [[large-language-models|Large Language Models]]
- [[agent-runtime|Agent Runtime]]
- [[llm-safety-interpretability|LLM Safety and Interpretability]]

## Open questions

- Which multimodal tasks require tool-using agents rather than model-only inference?
- How should visual evidence be stored and audited in agent traces?
- Which evaluation patterns are needed for dynamic visual tooling?
