...
def parse_checkins(path: Path) -> List[DayRecord]:
    ...
    for raw in lines:
        ...
        if current_section == 'training' and current_session:
            ...
            if km:
                key, value = km.groups()
                current_session.metrics[normalize_key(key)] = value.strip()
            continue
        ...
        if current_section == 'meals / intake':
            ...
            if mh:
                finalize_meal()
                current_meal = Meal(slot=mh.group(1).strip())
                continue
            if current_meal:
                ...
                if km:
                    ...
                    if key == 'foods':
                        current_meal.foods_text = value.strip()
                    elif key == 'calories':
                        current_meal.estimated_calories = parse_float(value)
                    elif key == 'protein':
                        current_meal.estimated_protein_g = parse_float(value)
                    elif key == 'notes':
                        current_meal.notes = value.strip()
                continue
    finalize_record()
    return records
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    ''')
    columns = {row[1] for row in conn.execute("PRAGMA table_info(plan_versions)").fetchall()}
    if 'plan_type' not in columns:
        conn.execute('ALTER TABLE plan_versions ADD COLUMN plan_type TEXT')
    conn.commit()
    ...
    if record.event_type == 'daily':
        ...
        for line in record.summary_lines:
            if line.lower().startswith('plan slot:'):
                ...
                break
        conn.execute(
            '''INSERT INTO days(date, plan_slot, actual_training_status, nutrition_status, notes, source_note, last_synced_at)
            ...
            (record.date, plan_slot, training_status, nutrition_status, notes or None, source_note),
        )
        ...
        for session in record.sessions:
            ...
            cursor = conn.execute(
                '''INSERT INTO training_sessions(
                       date, session_index, type, theme, duration_seconds, calories_total, calories_active,
                       avg_hr, max_hr, intensity, exertion_level, evaluation, recovery_notes, raw_metrics_json, source_note
                   ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (
                    record.date,
            ...
            for ex in session.exercises:
                conn.execute(
                    '''INSERT INTO exercises(session_id, set_order, exercise_name, raw_text, weight_value, reps, duration_seconds, distance_m, pace_text)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                    (session_id, ex.set_order, ex.name, ex.raw_text, ex.weight_value, ex.reps, ex.duration_seconds, ex.distance_m, ex.pace_text),
                )
        for meal in record.meals:
            conn.execute(
                '''INSERT INTO meals(date, meal_slot, foods_text, estimated_calories, estimated_protein_g, notes, source_note)
                   VALUES (?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(date, meal_slot, source_note) DO UPDATE SET
                     ...
                     notes=excluded.notes''',
                (record.date, meal.slot, meal.foods_text, meal.estimated_calories, meal.estimated_protein_g, meal.notes, source_note),
            )
    else:
...
def sync_plans(conn: sqlite3.Connection) -> List[dict]:
    ...
    for path, default_status in plan_sources:
        ...
        cursor = conn.execute(
            '''INSERT INTO plan_versions(version_label, effective_date, status, plan_type, source_note, summary)
               VALUES (?, ?, ?, ?, ?, ?)
        ...
        for slot in slots:
            conn.execute(
                '''INSERT INTO plan_slots(plan_version_id, weekday, weekday_order, slot_name, details)
                   VALUES (?, ?, ?, ?, ?)''',
                (plan_id, slot['weekday'], WEEKDAY_ORDER.get(slot['weekday']), slot['slot_name'], slot['details']),
            )
